...
class GithubProjectLinkColumn(tables.columns.Column):
    ...
    def render(self, value):
        return format_html(
            '<a href="https://github.com/{}" target="_blank">{}</a>', value, value
        )
...
class TaskTable(tables.Table):
    ...
    def render_issue_number(self, record):
        ...
        return format_html(
            f'<a href="{record.comment_url}" target="_blank">#{issue_number}</a>'
        )
...
class EventTable(tables.Table):
    ...
    def render_target(self, record):
        ...
        if record.action in github_issue_targets:
            ...
            return format_html(
                '<a href="{}" target="_blank">#{}</a>', issue_url, record.target
            )
        elif record.action == "clone_repo":
            repo_url = f"https://github.com/{record.task.github_project}"
            return format_html(
                '<a href="{}" target="_blank">{}</a>', repo_url, record.target
            )
        elif record.action == "commit_changes":
            commit_url = f"https://github.com/{record.task.github_project}/commit/{record.target}"
            return format_html(
                '<a href="{}" target="_blank">{}</a>', commit_url, record.target
            )
        elif record.action in ["push_branch", "checkout_pr_branch"]:
            branch_url = (
                f"https://github.com/{record.task.github_project}/tree/{record.target}"
            )
            return format_html(
                '<a href="{}" target="_blank">{}</a>', branch_url, record.target
            )
        elif record.action == "comment_on_issue":
            issue_id = (
                record.task.pr_number
                if record.task.pr_number
                else record.task.issue_number
            )
            return format_html(
                '<a href="{}" target="_blank">#{}</a>',
                record.task.response_comment_url,
                issue_id,
            )
        else:
...
class EventUndoTable(EventTable):
    ...
    def render_reversible(self, record):
        ...
        if record.reversible and not record.reversed:
            return format_html(
                '<input type="checkbox" name="reversible" value="{}" checked>',
                record.id,
            )
        else:
            return format_html(
                '<input type="checkbox" name="reversible" value="{}" disabled>',
                record.id,
            )
...
class CostItemTable(tables.Table):
    ...
    def render_credits(self, value):
        ...
        return format_html(
            '<span class="badge rounded-pill bg-dark"><i class="fa-solid fa-coins"></i> {}</span>',
            usd_string,
        )
